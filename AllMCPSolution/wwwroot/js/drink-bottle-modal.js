(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const noteOnlyField = form.querySelector('.drink-bottle-note-only-field');
        const noteOnlyToggle = form.querySelector('.drink-bottle-note-only-toggle');
        const noteOnlyHintId = noteOnlyToggle?.getAttribute('aria-describedby');
        const noteOnlyHint = noteOnlyHintId ? document.getElementById(noteOnlyHintId) : null;
        ...
        const setNoteOnlyState = (isNoteOnly) => {
            if (form) {
                if (isNoteOnly) {
            ...
            if (noteOnlyField) {
                noteOnlyField.classList.toggle('is-active', Boolean(isNoteOnly));
            }

            if (noteOnlyHint) {
                noteOnlyHint.classList.toggle('is-active', Boolean(isNoteOnly));
            }

            if (noteOnlyToggle) {
                noteOnlyToggle.checked = Boolean(isNoteOnly);
            }
            ...
            if (dateField) {
                dateField.classList.toggle('is-disabled', Boolean(isNoteOnly));
            }
        };

        const updateNoteOnlyMode = () => {
            setNoteOnlyState(Boolean(noteOnlyToggle?.checked));
        };
        ...
        noteOnlyToggle?.addEventListener('change', () => {
            contextState.noteOnly = Boolean(noteOnlyToggle?.checked);
            updateNoteOnlyMode();
            showFeedback('');
        });
        ...
        const setLoading = (state) => {
            ...
            setElementDisabled(noteOnlyToggle, state);
        ...
        form.addEventListener('submit', async (event) => {
            ...
            const isNoteOnly = Boolean(noteOnlyToggle?.checked);
            contextState.noteOnly = isNoteOnly;
