...
public class GetArtworkSalesPriceVsEstimateTool// : IToolBase, IMcpTool
{
    ...
    public GetArtworkSalesPriceVsEstimateTool(IArtworkSaleQueryRepository repo)
    {
        _repo = repo;
    }

    public string Name => "get_artwork_sales_price_vs_estimate_timeseries";
    public string Description => "Returns ratios and deltas showing hammer price relative to low/high estimates, including position within range.";
    ...
    public async Task<object> ExecuteAsync(Dictionary<string, object>? parameters)
    {
        ...
        var list = sales.Select(s =>
        {
            ...
            var hp = s.HammerPrice;
            decimal? ratioToLow = low > 0 ? hp / low : null;
            decimal? ratioToHigh = high > 0 ? hp / high : null;
            decimal deltaToLow = hp - low;
            decimal deltaToHigh = hp - high;
            decimal? positionInRange = (high > low) ? (hp - low) / (high - low) : (decimal?)null;
            decimal? percentOverHigh = (hp > high && high > 0) ? (hp - high) / high : null;
    ...
    {
        Name = Name,
        Title = "Price vs estimate timeseries",
        Description = Description,
