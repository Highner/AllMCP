...
public sealed class CellarTrackerImportService : ICellarTrackerImportService
{
    ...
    private static readonly Regex ConsumptionStatusSpanRegex = new(
        "<span[^>]*class=['\"][^'\"]*\\bel\\s+con[^'\"]*['\"][^>]*>(?<content>.*?)</span>",
        RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);

    private static readonly Regex ConsumptionDateSpanRegex = new(
        "<span[^>]*class=['\"][^'\"]*\\bel\\s+dat[^'\"]*['\"][^>]*>(?<content>.*?)</span>",
        RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);

    private static readonly Regex ConsumptionNoteSpanRegex = new(
        "<span[^>]*class=['\"][^'\"]*\\bel\\s+not[^'\"]*['\"][^>]*>(?<content>.*?)</span>",
        RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
